...
class AIAutoOptimizer:
    ...
    def __init__(self, decision_module, simulation_log='simulation_trades.log', real_log='real_trades.log', interval=60):
        ...
        self.decision_module.gemini_analyzer = None # Sera injecté
        self.running = False
        self.last_sim_len = 0
    ...
    def analyze_and_adjust(self):
        ...
        if self.gemini_analyzer:
            completed_log_path = "completed_trades.log"
            if os.path.exists(completed_log_path):
                logger.info("Lancement de l'analyse IA des trades complétés...")
                # On ne bloque pas le thread principal pour l'appel IA
                asyncio.create_task(self.gemini_analyzer.analyze_logs(log_path=completed_log_path))
