...
class DecisionModule:
    ...
    def __init__(self, order_executor: Any, buy_amount_sol: float, sell_multiplier: float, simulation_mode: bool = False):
        ...
        :param order_executor: module d'exécution des ordres (buy/sell)
        :param buy_amount_sol: montant à investir par trade
        :param sell_multiplier: multiplicateur de take profit
        :param simulation_mode: True pour la simulation, False pour le réel
        """
        ...
        self.creator_cache_file = "successful_creators.json"
        self.successful_creator_cache = self._load_creator_cache()
    ...
    def _load_creator_cache(self) -> set:
        """Charge le cache des créateurs performants depuis un fichier."""
        try:
            with open(self.creator_cache_file, "r", encoding="utf-8") as f:
                data = json.load(f)
                logger.info(f"Cache de {len(data)} créateurs performants chargé.")
                return set(data)
        except (FileNotFoundError, json.JSONDecodeError):
            return set()

    def _save_creator_cache(self):
        """Sauvegarde le cache des créateurs performants sur le disque."""
        with open(self.creator_cache_file, "w", encoding="utf-8") as f:
            json.dump(list(self.successful_creator_cache), f, indent=2)
        logger.info(f"Cache de créateurs performants sauvegardé ({len(self.successful_creator_cache)} adresses).")
    ...
    async def process_new_token_candidate(self, token_mint_address: str, current_price: float) -> None:
        ...
        try:
            ...
            if self.simulation_mode:
                # On évite les vérifications réseau pour aller très vite.
                logger.info(f"[SIMULATION FAST BUY] Achat simulé de {token_mint_address}")
                ...
                return # On arrête ici pour le mode simulation
            # Vérification prioritaire: si le créateur est dans le cache de confiance en mode réel
            if not self.simulation_mode:
                ...
                if is_trusted_creator:
                    logger.success(f"[TRUSTED BUY] Achat prioritaire du token {token_mint_address} car son créateur est dans la liste de confiance.")
                    ...
                    if success:
                        ...
                        logger.success(f"Successfully bought trusted token {token_mint_address}.")
                        if hasattr(self, 'realtime_analyzer'):
                            await self.realtime_analyzer.start_monitoring_token(token_mint_address)
                    return # Fin du traitement pour cet achat prioritaire
            ...
            montant_total = self.buy_amount_sol + 0.01
            ...
            if not is_sellable:
                ...
                return
            # Achat immédiat
            logger.info(f"[FAST BUY] Achat de {self.buy_amount_sol} SOL de {token_mint_address}")
            success = await self.order_executor.execute_buy(token_mint_address, self.buy_amount_sol)
            if success:
                creator_wallets = await self._detect_creator_wallets(token_mint_address)
                self.held_tokens[token_mint_address] = {
    ...
    async def _detect_creator_wallets(self, token_mint_address: str):
        ...
        try:
            ...
            signatures_resp = await self.order_executor.async_client.get_signatures_for_address(PublicKey(token_mint_address), limit=10, commitment="confirmed")
    ...
    async def _creator_wallet_selling(self, token_mint_address: str, creator_wallets: list) -> bool:
        ...
        async def check_wallet(wallet):
            try:
                signatures_resp = await self.order_executor.async_client.get_signatures_for_address(PublicKey(wallet), limit=5)
                signatures = signatures_resp.value
    ...
    def _log_completed_trade(self, token_mint_address: str, sell_price: float):
        ...
        with open("completed_trades.log", "a", encoding="utf-8") as f:
    ...
    async def _execute_sale(self, token_mint_address: str) -> None:
        ...
        try:
            if self.simulation_mode:
                ...
                # Simuler un prix de vente (peut être amélioré avec un vrai flux de prix)
                # Pour l'instant, on utilise le prix qui a déclenché la vente.
                # On pourrait récupérer le prix actuel ici si on avait un flux de prix pour la simulation.
                current_price = self.held_tokens[token_mint_address]['buy_price'] * self.sell_multiplier # Exemple
                ...
                # Note: execute_sell devrait retourner le prix de vente réel pour un logging précis
                ...
                if success_data and success_data.get("success"):
                    ...
                    # Utiliser le prix de vente réel si disponible, sinon le prix d'achat comme fallback
                    sell_price = success_data.get("price", self.held_tokens.get(token_mint_address, {}).get("buy_price", 0))
